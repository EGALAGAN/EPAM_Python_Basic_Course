    ...
    def publish(self):
        ...
        for __js_pbl in self.pbl_js_body["publish"]:
            ...
            if __js_pbl["title"] == 'Ad':
                __my_obj = ClassAd()
                __my_obj.pbl_text = __js_pbl["text"]
                __my_obj.pbl_expiration_date = datetime.datetime.strptime(__js_pbl["expiration_date"], __my_obj.date_format)
                __my_obj.get_pbl_left_days()
                __my_obj.upload_pbl(__my_obj.pbl_title, __my_obj.pbl_text,
                                  'Actual until: ' + __my_obj.pbl_expiration_date.strftime(__my_obj.date_format) + ', ' + (
                                      __my_obj.pbl_left_days) + ' days left', '------------------------------')

            if __js_pbl["title"] == 'News':
                __my_obj = ClassNews()
                __my_obj.pbl_city = __js_pbl["city"]
                __my_obj.pbl_text = __js_pbl["text"]
                __my_obj.get_pbl_date()
                __my_obj.upload_pbl(__my_obj.pbl_title, __my_obj.pbl_text, __my_obj.pbl_city + ', ' + __my_obj.pbl_date,
                                  '------------------------------')

            if __js_pbl["title"] == 'Weather':
                __my_obj = ClassWeather()
                __my_obj.pbl_city = __js_pbl["city"]
                __my_obj.pbl_temperature = __js_pbl["temperature"]
                __my_obj.get_pbl_final_mood()
                __my_obj.upload_pbl(__my_obj.pbl_title, __my_obj.pbl_city,
                                  'The temperature for today is ' + str(__my_obj.pbl_temperature) + ', ' + str(
                                      __my_obj.pbl_final_mood), '------------------------------')
        self.remove_file(self.pbl_full_path_name)
...
def my_prg_and_statistic():
    ...
    print(
        'What kind of publication do you want to publish?\n(1 - News, 2 - Advertising, 3 - Weather, 4 - From file, 5 - Refresh statistic, 6 - From JSON file)\nType a number:')
...
#     ]
# }
# json.dump(a, open('my_json.json', 'w'))
